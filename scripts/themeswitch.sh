...
# vs code
sed -i "/workbench.colorTheme/c\    \"workbench.colorTheme\": \"${ThemeSet}\"," $ConfDD/Code/User/settings.json


# kitty
ln -fs $ConfDD/kitty/themes/${ThemeSet}.conf $ConfDD/kitty/themes/theme.conf
killall -SIGUSR1 kitty


# qt5ct
sed -i "/^color_scheme_path=/c\color_scheme_path=$ConfDD/qt5ct/colors/${ThemeSet}.conf" $ConfDD/qt5ct/qt5ct.conf
IconSet=`awk -F "'" '$0 ~ /gsettings set org.gnome.desktop.interface icon-theme/{print $2}' $ConfDir/themes/${ThemeSet}.conf`
sed -i "/^icon_theme=/c\icon_theme=${IconSet}" $ConfDD/qt5ct/qt5ct.conf
...
dunstify $ncolor "theme" -a "    ${ThemeSet}" -i "$ConfigDir/dunst/icons/paint.svg" -r 91190 -t 2200
